//
//  PatientCardSeisanFinalizeViewModel.swift
//  seisan
//
...
final class PatientCardSeisanFinalizeViewModel: ObservableObject {
    ...
    private func finalize() {
        ...
        Task {
            do {
                ...
                // 各種インフラを生成
                let smaregiRepo = SmaregiPlatformRepository(setting: self.appSetGetSvc.getMustAppSetting().smaregiSetting!)
                var isRegisterShunoEnable = false
                var orcaRepo: ORCARepository?
                if self.appSetGetSvc.getMustAppSetting().patientCardSeisanSetting!.isORCAPaymentEnable {
                    // ORCA入金が有効な場合
                    isRegisterShunoEnable = true
                    orcaRepo = ORCARepository(setting: self.appSetGetSvc.getMustAppSetting().orcaSetting!)
                }
                
                // 取引終了処理サービスを実行
                try await PatientCardSeisanFinalizeService(
                    ...
                    isRegisterShunoEnable: isRegisterShunoEnable,
...
extension PatientCardSeisanFinalizeViewModel {
    /// 取引終了処理画面表示ステータス種別
